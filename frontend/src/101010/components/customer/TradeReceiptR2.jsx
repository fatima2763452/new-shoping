...
const TradeReceiptR2 = ({
  ...
  const displayPrice = isEditing ? (parseFloat(editData.price) || 0) : (isExit ? trade.price : (trade.entryPrice || trade.price || 0));
  const displayLtp = isEditing ? (parseFloat(editData.ltp) || 0) : (isExit ? trade.ltp : trade.price);
  ...
  return (
    <div className={`p-6 sm:p-8 transition-colors duration-300 ${isDark ? 'bg-[#0b1329] text-slate-200' : 'bg-white text-slate-800'}`}>
      ...
      <div className={`pb-5 border-b border-dashed ${isDark ? 'border-slate-800/80' : 'border-slate-200'}`}>
        ...
        <div className="flex gap-12 mt-6 text-xs">
          <div>
          ...
          <div className={`border-l pl-10 ${isDark ? 'border-slate-800/80' : 'border-slate-200'}`}>
            <span className={`block font-semibold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Date</span>
            <span className={`block font-bold mt-1 text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{formatDate(trade.date)}</span>
          </div>
        </div>
      </div>
      ...
      <div className={`flex justify-between items-center p-5 rounded-xl border mb-5 ${
        ...
        <div className="flex-1 min-w-0">
          <span className={`block text-[10px] font-bold uppercase tracking-wider mb-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>P&L</span>
          <span className={`text-sm font-bold whitespace-nowrap ${displayTotalPnl >= 0 ? 'text-emerald-400' : 'text-rose-500'}`}>
            {displayTotalPnl >= 0 ? '+' : ''}{formatCurrency(displayTotalPnl)}
          </span>
        </div>
        ...
        <div className={`flex-1 min-w-0 pl-6 border-l ${isDark ? 'border-slate-800/80' : 'border-slate-200'}`}>
          <span className={`block text-[10px] font-bold uppercase tracking-wider mb-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Avg. Buy Price</span>
          {isEditing ? (
      ...
      <div className={`flex justify-between items-stretch p-5 rounded-xl border mb-5 ${
        ...
        <div className="flex-1 pr-6">
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-bold ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>09:15:23 AM</span>
            </div>
          </div>
        </div>
        <div className={`flex-1 pl-6 border-l ${isDark ? 'border-slate-800/80' : 'border-slate-200'}`}>
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <div className="flex justify-between">
              <span className={isDark ? 'text-slate-500' : 'text-slate-400'}>Time:</span>
              <span className={`font-bold ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>10:45:31 AM</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`flex justify-between items-center p-5 rounded-xl border mb-6 ${
        ...
        <div className={`flex-1 min-w-0 pl-6 border-l ${isDark ? 'border-slate-800/80' : 'border-slate-200'}`}>
          <span className={`block text-[10px] font-bold uppercase tracking-wider mb-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>NET P&L</span>
