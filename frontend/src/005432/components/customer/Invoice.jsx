...
export default function Invoice() {
    ...
    const handleDownloadPDF = async () => {
        try {
            ...
            pdf.text('Total Profit', rightCardX + 10, cursorY + 56);
            pdf.text('Total Loss', rightCardX + 10, cursorY + 68);
            pdf.text('Total Brokerage', rightCardX + 10, cursorY + 80);
            ...
            pdf.text(`Rs. ${formatPDFNumber(summary.totalProfit)}`, rightCardX + summaryCardW - 10, cursorY + 56, { align: 'right' });

            // Total Loss (red)
            pdf.setTextColor(239, 68, 68);
            pdf.text(`-Rs. ${formatPDFNumber(summary.totalLoss)}`, rightCardX + summaryCardW - 10, cursorY + 68, { align: 'right' });

            // Total Brokerage
            pdf.setTextColor(15, 23, 42);
            pdf.text(`Rs. ${formatPDFNumber(summary.totalBrokerage)}`, rightCardX + summaryCardW - 10, cursorY + 80, { align: 'right' });

            // Total Profit / Loss box (black border, no radius, no background)
            pdf.setDrawColor(0, 0, 0);
            pdf.setLineWidth(1.5);
            pdf.setFillColor(255, 255, 255);
            pdf.rect(rightCardX + 8, cursorY + 92, summaryCardW - 16, 22, 'FD');
            pdf.setLineWidth(0.5);

            pdf.setFont(activeFont, 'bold');
            pdf.setFontSize(7.5);
            pdf.setTextColor(15, 23, 42);
            pdf.text('TOTAL PROFIT / LOSS', rightCardX + 14, cursorY + 106);
            pdf.setFontSize(8.5);
            const netPnlVal = summary.netPnl;
            pdf.setTextColor(netPnlVal >= 0 ? 0 : 239, netPnlVal >= 0 ? 176 : 68, netPnlVal >= 0 ? 80 : 68);
            const pdfNetPnlStr = netPnlVal >= 0 ? `Rs. ${formatPDFNumber(netPnlVal)}` : `-Rs. ${formatPDFNumber(Math.abs(netPnlVal))}`;
            pdf.text(pdfNetPnlStr, rightCardX + summaryCardW - 14, cursorY + 106, { align: 'right' });
